...
{
	/* we handle loading driver in init script ? */
...
int wifi_connect_to_supplicant()
{
    ...
    if (access(IFACE_DIR, F_OK) == 0) {
		LOGI("wpa_supplicant control access possible");
        snprintf(ifname, sizeof(ifname), "%s/%s", IFACE_DIR, iface);
    } else {
		LOGI("wpa_supplicant control access impossible");
...
int wifi_wait_for_event(char *buf, size_t buflen)
{
    ...
    if (result == 0 && nread == 0) {
        ...
        LOGI("Received EOF on supplicant socket\n");
